...
class WeightStandardization:
    ...
    def compute_weight(self, module: T_module) -> torch.Tensor:
        weight = getattr(module, self.name + "_")
    ...
    def apply(
        ...
        module.register_parameter(name + '_', Parameter(weight.data))
        if learnable_gain:
            ...
            module.register_parameter(name + '_gain', Parameter(g.data))
        else:
            module.register_buffer(name + "_gain", None)
        
        if scale is not None:
            s = torch.ones(1, dtype=weight.dtype, device=weight.device) * scale
            module.register_buffer("scale", s)
        else:
            module.register_buffer("scale", None)
    ...
    def remove(self, module: T_module) -> None:
        ...
        del module._parameters[self.name + '_']
        del module._parameters[self.name + '_gain']
