...
class EuclideanCodebook(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x):
        shape, dtype = x.shape, self.embed.dtype        # x.dtype: float16 / embed.dtype: float32
        ...
        if self.training:
            with autocast(enabled=False):
                if self.distributed:
                    ...
                    bucket = torch.empty(ema_num_numel + ema_embed_numel, dtype=dtype)
...
class ResidualVQ(nn.Module):
    ...
    def forward(self, x, n=None):
        ...
        for idx, layer in enumerate(self.layers[:high]):
            calculate_commitment_loss = (idx == high - 1)
            quantized, num_replace, loss = layer(residual, calculate_commitment_loss)
