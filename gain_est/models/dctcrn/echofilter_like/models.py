...
class DCTCRN(nn.Module):
    def __init__(
        ...
        self.enhance_inter = nn.LSTM(
                input_size= rnn_channels * 2,
                hidden_size=rnn_channels,
                num_layers=rnn_layers,
                dropout=0.5,
                bidirectional=False,
        ...
        self.enhance_inter.flatten_parameters()
        self.enhance_last.flatten_parameters()
    ...
    def forward(self, far, mix):
        ...
        x = torch.cat((x_m, x_f), -1)

        x_mid, _ = self.enhance_inter(x)

        x_last = torch.cat((x_m, x_mid), -1)

        x_last, _ = self.enhance_last(x_last)
        x_last = self.tranform_mix(x_last)

        x_last = torch.reshape(x_last, [lengths, batch_size, channels, dims])
        x_last = x_last.permute(1, 2, 3, 0)

        for idx in range(len(self.decoder_speech)):
            x_last = torch.cat([x_last, mix_encoder_out[-1 - idx]], 1)
            x_last = self.decoder_speech[idx](x_last)
            #x_f = torch.cat([x_f, far_encoder_out[-1 - idx]], 1)
            #x_f = self.decoder_echo[idx](x_f)
        
        res = self.mask(x_last, mix_spec).squeeze(1)
