...
class GConv(nn.Module):
    def __init__(
        ...
        self.conv1 = CausalConv2d(in_channel, out_channel, kernel_size, stride)
        self.conv2 = CausalConv2d(in_channel, out_channel, kernel_size, stride)
...
class TrGConv(nn.Module):
    def __init__(
        ...
        self.Trconv1 = CausalConvTranspose2d(in_channel, out_channel, kernel_size, stride)
        self.Trconv2 = CausalConvTranspose2d(in_channel, out_channel, kernel_size, stride)
        self.BN = nn.BatchNorm2d(out_channel)
        self.prelu = nn.PReLU()
    
    def forward(self, sig):
        sig1 = self.Trconv1(sig)
        sig2 = self.Trconv2(sig)
        sig2 = torch.sigmoid(sig2)
        res = sig1 * sig2
