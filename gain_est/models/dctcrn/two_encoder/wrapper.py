...
class ModelWrapper(AudioModelWrapper):
    ...
    def infer_epoch(self, dataloader):
        ...
        for idx, batch in enumerate(dataloader, start=1):
            ...
            print("\tPESQ_EST={0:.2f}".format(pesq_est))
            print("\tPESQ_MIX={0:.2f}".format(pesq_mix))
            print("\tPESQ_DIFF={0:.2f}".format(pesq_est-pesq_mix))
            # Compute SI-SNRi
        ...
        print("Average PESQ(Enhanced) : {0:.2f}".format(total_pesq_est / (total_cnt)))
        ...
        summary['scalars']['pesq_diff'] = (total_pesq_est / total_cnt)
