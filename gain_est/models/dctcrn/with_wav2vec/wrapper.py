...
from transformers import Wav2Vec2Processor ## intelligibility test -- 220421
...
class ModelWrapper(AudioModelWrapper):
    ...
    def train_epoch(self, dataloader):
        ...
        for idx, batch in enumerate(dataloader, start=1):
            ...
            with amp.autocast(enabled=self.fp16):
                ...
                rep_est = self.processor(wav_hat.detach().cpu().numpy(), sampling_rate=16000, return_tensors='pt').input_values.to('cuda')
                rep_clean = self.processor(near.detach().cpu().numpy(), sampling_rate=16000, return_tensors='pt').input_values.to('cuda')
                loss *= 0.05
                loss += F.mse_loss(rep_est, rep_clean) * 0.95
    ...
    def valid_epoch(self, dataloader):
        ...
        summary_scalars = {
            "loss/si_snr": loss,
            "learnable_param_1": self.model.w1.item(), ####### Learnable parameter로 실험할 시
            "learnable_param_2" : self.model.w2.item()
        }
        if self.loss_aux is not None:
