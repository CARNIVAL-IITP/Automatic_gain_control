...
def get_dct_filter(y: Tensor, N: int, win_size: Optional[int], win_type: int, window: Optional[Tensor]) -> Tensor:
    global dct_filter
...
def stdct(y: Tensor, N: int, hop_size: int, win_size: Optional[int] = None, center: bool = False,
          win_type: str = "hann", window: Optional[Tensor] = None) -> Tensor:
...
def istdct(y: Tensor, N: int, hop_size: int, win_size: Optional[int] = None, center: bool = False,
           win_type: str = "hann", window: Optional[Tensor] = None) -> Tensor:
    ...
    N_win_dtype_device = f"{N}_{win_size}_{y.dtype}_{y.device}"
    window_square = dct_window_square[N_win_dtype_device].view(1, -1, 1).expand(y.size(0), -1, y.size(-1))
...
def stft_new(y: Tensor, n_fft: int, hop_size: int, win_size: int, center: bool = False, magnitude: bool = True) -> Tensor:
    if torch.min(y) < -1.:
...
def spec_to_mel(spec: Tensor, n_fft: int, num_mels: int, sampling_rate: int, fmin: float = 0.0,
                fmax: Optional[float] = None, clip_val: float = 1e-5, log: bool = True, norm: str = 'slaney') -> Tensor:
    global mel_basis
    ...
    if nmel_nfft_fmax_norm_dtype_device not in mel_basis:
        mel = librosa_mel_fn(sampling_rate, n_fft, num_mels, fmin, fmax, norm=norm)
...
def mel_spectrogram(y: Tensor, n_fft: int, num_mels: int, sampling_rate: int, hop_size: int, win_size: int,
                    fmin: float = 0.0, fmax: Optional[float] = None, center: bool = False,
                    clip_val: float = 1e-5, log: bool = True, norm: str = 'slaney') -> Tensor:
    ''' wav to mel
