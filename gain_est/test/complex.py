...
class Encoder_farend(nn.Module):
    def __init__(self):
        ...
        self.conv1 = Conv2Dblock(4, 64, kernel_size=(5, 3),stride=(4, 1),padding=(2,2))
    def forward(self,inputs):
...
class Encoder_nearend(nn.Module):
    def __init__(self):
        ...
        self.conv1 = Conv2Dblock(4, 16, kernel_size=(5, 3), stride=(1, 1), padding=(2, 2))
        self.conv2 = Conv2Dblock(16, 32, kernel_size=(5, 3), stride=(2, 1), padding=(2, 2))
        self.conv3 = Conv2Dblock(32, 64, kernel_size=(5, 3), stride=(2, 1), padding=(2, 2))
...
class Decoder_farend(nn.Module):
    def __init__(self):
        super(Decoder_farend,self).__init__()
        self.deconv1 = ComplexConvTranspose2d(64,4,kernel_size=(3,5),stride=(4,1),padding=(1,2))
        self.conv1_1 = Conv2Dblock(8, 4, kernel_size=(1, 1), stride=(1, 1))
        self.conv1_2 = Conv2Dblock(8, 4, kernel_size=(1, 1), stride=(1, 1))
        self.conv_2 = Conv2Dblock(4, 2, kernel_size=(1, 1), stride=(1, 1))
    def forward(self,inputs,encoder_input):
        output_11 = self.deconv1(inputs)
        output_12 = complex_cat([output_11,encoder_input[0]],axis=1)
        output_13 = self.conv1_1(output_12)
        output_14 = output_13*encoder_input[0]
        output_15 = complex_cat([output_14,output_11],axis=1)
        output_16 = self.conv1_2(output_15)
        output_1 = output_16+ output_11

        output = self.conv_2(output_1)
...
class Decoder_nearend(nn.Module):
    def __init__(self):
        ...
        self.deconv1 = ComplexConvTranspose2d(64, 32, kernel_size=(5, 3), stride=(2, 1), padding=(2, 1))
        self.deconv2 = ComplexConvTranspose2d(32,16,kernel_size=(5,3),stride=(2,1),padding=(2,1))
        self.deconv3 = ComplexConvTranspose2d(16,4,kernel_size=(5,3),stride=(1,1),padding=(2,1))
        ...
        self.conv3_1 = Conv2Dblock(8,4,kernel_size=(1,1),stride=(1,1))
        self.conv3_2 = Conv2Dblock(8,4, kernel_size=(1, 1), stride=(1, 1))

        self.conv_4 = Conv2Dblock(4,2,kernel_size=(1,1),stride=(1,1))

    def forward(self,inputs,encoder_input):
        output_11 = self.deconv1(inputs)

        output_12 = complex_cat([output_11,encoder_input[2]],axis=1)
        output_13 = self.conv1_1(output_12)
...
class RA_Block(nn.Module):
    def __init__(self,
        ...
        self.res_block1 = ComplexConv2d(64,64,kernel_size=(7,5),stride=(1,1),padding=(3,4))
        self.res_block2 = ComplexConv2d(64,64,kernel_size=(7,5),stride=(1,1),padding=(3,4))
...
class interaction_block(nn.Module):
    ...
    def forward(self,nearend_feature,echo_feature):
        output = complex_cat([nearend_feature,echo_feature],axis=1)
        output = self.conv_block(output)
...
class echo_gen(nn.Module):
    ...
    def forward(self,farend,nearend):
        nearend_cspecs = self.get_cspecs(nearend)
        farend_cspecs = self.get_cspecs(farend)
        ...
        nearend_dec_output = torch.cat([nearend_dec_output[:,0],nearend_dec_output[:,1]],1)
        farend_dec_output = torch.cat([farend_dec_output[:,0],farend_dec_output[:,1]],1)

        nearend_speech_est = self.istft(nearend_dec_output)
        echo_est = self.istft(farend_dec_output)
